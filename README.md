...
## Notes
* In terms of simulation time, takes ~3 hours for small proteins like ubiquitin (8.6 kDa, 12.5 ns simulation time), ~4.5 hours for lysozyme (14 kDa, 15 ns simulation time), and ~15 hours for wheat agglutinin (35 kDa, 25 ns simulation time) using GPU acceleration with fairly old GPU's. As droplets are all-interacting, Larger proteins can take **much** longer.
...
* In terms of forcefield choice, **simESI is only capable of running simulations with the ```charmm36``` forcefield** although incorporating OPLS/AA is currently in progress. Additionally, given the number of non-standard molecules, simESI must be run with the  ```charmm36``` version contained with ```simESI-main/share```.
...
* ```--hpc``` *(Optional)* Choose to run with more ```gmx mdrun``` params. This assumes 1 node and GPU acceleration. If using different resources/setup, will need to manually modify some ```subprocess``` calls within simESI related to ```gmx mdrun``` to reflect the nodes/thread you are pulling. Default of ```no```.
